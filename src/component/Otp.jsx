...
const Otp = () => {
    ...
    const handleotp = async () => {
        ...
        try {
            const response = await fetch('https://sparkapi-stage.dvaramoney.com/c360/api/v1/verifyOtp', {
                method: 'POST',
                headers: {
    ...
    return (
        <div className='h-screen w-screen flex items-center justify-center bg-cover bg-center' style={{ backgroundImage: `url(${loginimage})` }}>
            <div className="flex flex-col items-center md:max-w-[423px] w-[380px] bg-white rounded-2xl shadow-lg p-6 sm:p-10">
                ...
                <button onClick={handleotp} type="button" className="mt-8 w-full max-w-80 h-11 rounded-full text-white text-sm bg-indigo-500 hover:opacity-90 transition-opacity">
                    Verify OTP
                </button>
            </div>
        </div>
